...
<head>
    ...
    <title>Create New Password | Educore Management Systems</title>
...
<body class="flex min-h-screen flex-col bg-surface text-on-surface">
    ...
    <main class="relative flex flex-grow items-center justify-center overflow-hidden bg-surface px-6 pb-12 pt-24">
        ...
        <div class="z-10 w-full max-w-[560px]">
            <div class="rounded-xl border border-outline-variant/15 bg-surface-container-lowest p-8 shadow-[0_10px_30px_rgba(25,27,34,0.06)] md:p-12">
                <div class="mb-10 text-center">
                    ...
                    <h1 class="mb-2 font-headline text-4xl font-extrabold tracking-tighter text-primary">Create a new password</h1>
                    <p class="mx-auto max-w-md text-sm font-medium text-on-surface-variant">
                        Set a strong new password for your EduCore account and return to your academic dashboard securely.
                    </p>
                </div>
                ...
                <form class="space-y-6" method="POST" action="{{ route('password.update') }}">
                    ...
                    <div class="space-y-2">
                        ...
                        <input class="w-full rounded-lg border-none border-b-2 border-transparent bg-surface-container-highest px-4 py-3.5 text-on-surface placeholder:text-outline transition-all focus:border-primary focus:ring-0 focus:ring-offset-0" id="email" name="email" placeholder="registrar@educore.edu" type="email" value="{{ old('email', $request->email) }}" required autofocus autocomplete="username" />
                    </div>

                    <div class="space-y-2">
                        <label class="block font-label text-sm font-semibold text-on-surface-variant" for="password">New password</label>
                        <input class="w-full rounded-lg border-none border-b-2 border-transparent bg-surface-container-highest px-4 py-3.5 text-on-surface placeholder:text-outline transition-all focus:border-primary focus:ring-0 focus:ring-offset-0" id="password" name="password" placeholder="Create a strong password" type="password" required autocomplete="new-password" />
                    </div>

                    <div class="space-y-2">
                        <label class="block font-label text-sm font-semibold text-on-surface-variant" for="password_confirmation">Confirm new password</label>
                        <input class="w-full rounded-lg border-none border-b-2 border-transparent bg-surface-container-highest px-4 py-3.5 text-on-surface placeholder:text-outline transition-all focus:border-primary focus:ring-0 focus:ring-offset-0" id="password_confirmation" name="password_confirmation" placeholder="Repeat your new password" type="password" required autocomplete="new-password" />
                    </div>
                ...
                <div class="mt-8 rounded-xl border border-outline-variant/15 bg-surface-container-low px-4 py-4 text-sm text-on-surface-variant">
                    This form is only valid for the reset link sent to your email. If the link has expired, request
                    <a class="font-semibold text-primary hover:underline decoration-2 underline-offset-4" href="{{ route('password.request') }}">a new reset link</a>.
                </div>
            </div>
            ...
            <div class="mt-8 flex justify-center">
                <div class="flex max-w-fit items-center gap-2 rounded-full border border-tertiary-container/20 bg-tertiary-container/10 px-4 py-2">
                    <span class="material-symbols-outlined text-sm text-tertiary-container">encrypted</span>
                    <span class="text-[10px] font-bold uppercase tracking-widest text-tertiary-container">Protected Reset Session</span>
                </div>
            </div>
        </div>
    </main>
